...
export async function updateProfile(formData: FormData) {
  ...
  try {
    ...
    const email =
      emailRaw && typeof emailRaw === "string" ? emailRaw : undefined;
...
export async function updateSettings(formData: FormData) {
  ...
  try {
    ...
    const aiSuggestionsEnabled =
      formData.get("aiSuggestionsEnabled") === "true";
    const reminderTimeStrRaw = formData.get("reminderTime");

    const timezone =
      timezoneRaw && typeof timezoneRaw === "string" ? timezoneRaw : undefined;
    const reminderTimeStr =
      reminderTimeStrRaw && typeof reminderTimeStrRaw === "string"
        ? reminderTimeStrRaw
        : undefined;
    ...
    if (
      reminderTimeStr &&
      (isNaN(parseInt(reminderTimeStr)) || parseInt(reminderTimeStr) < 0)
    ) {
      throw new Error("Reminder time must be a non-negative number");
    }

    const reminderTime = reminderTimeStr
      ? parseInt(reminderTimeStr)
      : undefined;
...
export async function getProfileData() {
  ...
  try {
    ...
    const googleIntegration = await GoogleIntegrationService.getUserIntegration(
      session.user.id,
    );
...
export async function connectGoogleCalendar() {
  ...
  const { url: authUrl } = await GoogleIntegrationService.getAuthUrl();
  redirect(authUrl);
}
