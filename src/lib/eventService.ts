...
export class EventService {
     ...
     */
    private static async formatLocalEvent(event: LocalEvent): Promise<UnifiedEvent> {
        return {
            id: `local_${event.id}`,
            title: event.title,
        ...
        try {
            ...
            if (eventId) {
                ...
                if (preferredOrigin === "google" && await this.hasGoogleIntegration(userId)) {
                    ...
                    console.log('Creating Google event:', { googleEventData, calendarId })
            ...
            throw new Error(`Failed to save event: ${error instanceof Error ? error.message : 'Unknown error'}`)
        }
    }
