...
export const UpdateEventSchema = CreateEventSchema.partial().omit({ title: true }).extend({
  title: z.string()
    .min(1, 'Title is required')
    .max(200, 'Title must be less than 200 characters')
    .trim()
...
export function validateEventUpdate(data: any) {
...
export function validateSettingsUpdate(data: any) {
  return UpdateSettingsSchema.safeParse(data)
}

export function validateAIChat(data: any) {
