...
export function EventForm({ isOpen, onClose, onSave, initialData, selectedDate }: EventFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      const startDate = initialData.startTime ? new Date(initialData.startTime) : selectedDate || new Date()
      const endDate = initialData.endTime ? new Date(initialData.endTime) : selectedDate || new Date()
      ...
      setFormData({
        ...
        endDate,
        endTime: initialData.endTime ? format(new Date(initialData.endTime), 'HH:mm') : '10:00',
  ...
  useEffect(() => {
    ...
    if (form) {
      const startDateInput = form.querySelector('input[name="startDate"]') as HTMLInputElement
      const endDateInput = form.querySelector('input[name="endDate"]') as HTMLInputElement
      ...
      if (startDateInput) startDateInput.value = formData.startDate.toISOString()
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px] max-h-[90vh] overflow-y-auto">
        ...
        <form action={handleSubmit} className="space-y-6 py-4">
          ...
          <input type="hidden" name="startDate" value={formData.startDate.toISOString()} />
          <input type="hidden" name="endDate" value={formData.endDate.toISOString()} />
          <input type="hidden" name="startTime" value={formData.startTime} />
          <input type="hidden" name="endTime" value={formData.endTime} />
          <input type="hidden" name="isAllDay" value={formData.isAllDay.toString()} />
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving}>
              {saving ? "Saving..." : (initialData?.id ? "Update Event" : "Create Event")}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
