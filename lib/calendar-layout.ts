...
export function packAllDaySpans<T>(spans: AllDaySpan<T>[]): AllDaySpan<T>[][] {
  ...
  for (const span of sorted) {
    const row = rows.find((existingRow) => existingRow.every((other) => other.endIdx < span.startIdx || other.startIdx > span.endIdx))
    if (row) {
      row.push(span)
    } else {
      rows.push([span])
 ...
 */
export function layoutOverlappingEvents(events: TimedEvent[]): Map<string, EventLayout> {
  ...
  const packCluster = (clusterEvents: TimedEvent[]) => {
    ...
    for (const event of clusterEvents) {
      ...
      for (let c = eventColumn + 1; c < columnCount; c++) {
        const conflicts = columns[c].some((other) => other.start < event.end && other.end > event.start)
        if (conflicts) break
        span++
      }
