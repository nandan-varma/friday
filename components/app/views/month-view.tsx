...
export function MonthView({
  ...
  const finishSelection = useCallback(() => {
    setRangeSelection((selection) => {
      ...
      const rangeStart =
        selection.start <= selection.current
          ? selection.start
          : selection.current;
      const rangeEnd =
        selection.start <= selection.current
      ...
      if (isSameDay(rangeStart, rangeEnd)) {
        onCreateEvent(
          set(rangeStart, { hours: 9, minutes: 0 }),
          set(rangeStart, { hours: 10, minutes: 0 }),
        );
      } else {
        onCreateEvent(startOfDay(rangeStart), endOfDay(rangeEnd), {
          allDay: true,
        });
      }
      return null;
  ...
  const isInSelection = (date: Date) => {
    ...
    const rangeStart =
      rangeSelection.start <= rangeSelection.current
          ...
          {days.map((day) => {
            ...
            return (
              <div
                ...
                onMouseDown={() =>
                  setRangeSelection({ start: day, current: day })
                }
                onMouseEnter={(e) => {
                  if (rangeSelection && e.buttons === 1) {
                    setRangeSelection((prev) =>
                      prev ? { ...prev, current: day } : prev,
                    );
                  }
                }}
